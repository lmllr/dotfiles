...
zstyle ':vcs_info:git*' stagedstr '%F{yellow}'$'\Ue729''%f '


# A list of formats, used when actionformats is not used (which is most of the time).
zstyle ':vcs_info:git*' formats "%F{11}%b%f%F{9}::%f%F{11}%12.12i%f %c%m" # hash & branch
...
zstyle ':vcs_info:git*' actionformats "%F{11}%b%f%F{9}::%f%F{11}%12.12i%f%F{9}//:%f%F{11}%a%f %c %F{cyan}%12.12m%f"

zstyle ':vcs_info:git*+set-message:*' hooks git-st git-stash git-dirty git-branch git-tag
